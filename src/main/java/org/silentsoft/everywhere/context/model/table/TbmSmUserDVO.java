package org.silentsoft.everywhere.context.model.table;

import java.math.BigDecimal;
import java.util.Date;

public class TbmSmUserDVO {
	
	private String userId;

	private String userPwd;

	private String userNm;

	private String engUserNm;

	private String empNo;

	private String emailAddr;

	private String singleId;

	private String deptCode;

	private String deptNm;

	private String engDeptNm;

	private String firmTel;

	private String mobileTel;

	private String fnlAccsDt;

	private String fnlAccsIp;

	private String acctStatusCode;

	private String acctStatusUpdDt;

	private String langCode;

	private BigDecimal pwdErrCnt;

	private String userIdValidDt;

	private String userTypeCode;

	private String secuPldgDt;

	private String gbmCode;

	private String corpCode;

	private String fctCode;

	private String mfgGrpCode;

	private String mfgTeamCode;

	private String mfgPartCode;

	private String outsVendCode;

	private String delYn;

	private String useYn;

	private Date etlUpdDt;

	private String fstRegDt;

	private String fstRegerId;

	private String fnlUpdDt;

	private String fnlUpderId;

	private String pwdValidDt;

	private String encptUserId;

	private String psnlInfoPldgDt;

	private String userRegRsnCode;

	private String userRegRsnDesc;

	private String sysPicYn;

	private String sysPicRoleDesc;

	private String pstnCode;

	private String acctNotchkYn;

	private String saveFctCode;

	private String photoUseYn;
	
	private String uniqueSeq;

	public String getUserId() {
		return userId;
	}

	public void setUserId(String userId) {
		this.userId = userId;
	}

	public String getUserPwd() {
		return userPwd;
	}

	public void setUserPwd(String userPwd) {
		this.userPwd = userPwd;
	}

	public String getUserNm() {
		return userNm;
	}

	public void setUserNm(String userNm) {
		this.userNm = userNm;
	}

	public String getEngUserNm() {
		return engUserNm;
	}

	public void setEngUserNm(String engUserNm) {
		this.engUserNm = engUserNm;
	}

	public String getEmpNo() {
		return empNo;
	}

	public void setEmpNo(String empNo) {
		this.empNo = empNo;
	}

	public String getEmailAddr() {
		return emailAddr;
	}

	public void setEmailAddr(String emailAddr) {
		this.emailAddr = emailAddr;
	}

	public String getSingleId() {
		return singleId;
	}

	public void setSingleId(String singleId) {
		this.singleId = singleId;
	}

	public String getDeptCode() {
		return deptCode;
	}

	public void setDeptCode(String deptCode) {
		this.deptCode = deptCode;
	}

	public String getDeptNm() {
		return deptNm;
	}

	public void setDeptNm(String deptNm) {
		this.deptNm = deptNm;
	}

	public String getEngDeptNm() {
		return engDeptNm;
	}

	public void setEngDeptNm(String engDeptNm) {
		this.engDeptNm = engDeptNm;
	}

	public String getFirmTel() {
		return firmTel;
	}

	public void setFirmTel(String firmTel) {
		this.firmTel = firmTel;
	}

	public String getMobileTel() {
		return mobileTel;
	}

	public void setMobileTel(String mobileTel) {
		this.mobileTel = mobileTel;
	}

	public String getFnlAccsDt() {
		return fnlAccsDt;
	}

	public void setFnlAccsDt(String fnlAccsDt) {
		this.fnlAccsDt = fnlAccsDt;
	}

	public String getFnlAccsIp() {
		return fnlAccsIp;
	}

	public void setFnlAccsIp(String fnlAccsIp) {
		this.fnlAccsIp = fnlAccsIp;
	}

	public String getAcctStatusCode() {
		return acctStatusCode;
	}

	public void setAcctStatusCode(String acctStatusCode) {
		this.acctStatusCode = acctStatusCode;
	}

	public String getAcctStatusUpdDt() {
		return acctStatusUpdDt;
	}

	public void setAcctStatusUpdDt(String acctStatusUpdDt) {
		this.acctStatusUpdDt = acctStatusUpdDt;
	}

	public String getLangCode() {
		return langCode;
	}

	public void setLangCode(String langCode) {
		this.langCode = langCode;
	}

	public BigDecimal getPwdErrCnt() {
		return pwdErrCnt;
	}

	public void setPwdErrCnt(BigDecimal pwdErrCnt) {
		this.pwdErrCnt = pwdErrCnt;
	}

	public String getUserIdValidDt() {
		return userIdValidDt;
	}

	public void setUserIdValidDt(String userIdValidDt) {
		this.userIdValidDt = userIdValidDt;
	}

	public String getUserTypeCode() {
		return userTypeCode;
	}

	public void setUserTypeCode(String userTypeCode) {
		this.userTypeCode = userTypeCode;
	}

	public String getSecuPldgDt() {
		return secuPldgDt;
	}

	public void setSecuPldgDt(String secuPldgDt) {
		this.secuPldgDt = secuPldgDt;
	}

	public String getGbmCode() {
		return gbmCode;
	}

	public void setGbmCode(String gbmCode) {
		this.gbmCode = gbmCode;
	}

	public String getCorpCode() {
		return corpCode;
	}

	public void setCorpCode(String corpCode) {
		this.corpCode = corpCode;
	}

	public String getFctCode() {
		return fctCode;
	}

	public void setFctCode(String fctCode) {
		this.fctCode = fctCode;
	}

	public String getMfgGrpCode() {
		return mfgGrpCode;
	}

	public void setMfgGrpCode(String mfgGrpCode) {
		this.mfgGrpCode = mfgGrpCode;
	}

	public String getMfgTeamCode() {
		return mfgTeamCode;
	}

	public void setMfgTeamCode(String mfgTeamCode) {
		this.mfgTeamCode = mfgTeamCode;
	}

	public String getMfgPartCode() {
		return mfgPartCode;
	}

	public void setMfgPartCode(String mfgPartCode) {
		this.mfgPartCode = mfgPartCode;
	}

	public String getOutsVendCode() {
		return outsVendCode;
	}

	public void setOutsVendCode(String outsVendCode) {
		this.outsVendCode = outsVendCode;
	}

	public String getDelYn() {
		return delYn;
	}

	public void setDelYn(String delYn) {
		this.delYn = delYn;
	}

	public String getUseYn() {
		return useYn;
	}

	public void setUseYn(String useYn) {
		this.useYn = useYn;
	}

	public Date getEtlUpdDt() {
		return etlUpdDt;
	}

	public void setEtlUpdDt(Date etlUpdDt) {
		this.etlUpdDt = etlUpdDt;
	}

	public String getFstRegDt() {
		return fstRegDt;
	}

	public void setFstRegDt(String fstRegDt) {
		this.fstRegDt = fstRegDt;
	}

	public String getFstRegerId() {
		return fstRegerId;
	}

	public void setFstRegerId(String fstRegerId) {
		this.fstRegerId = fstRegerId;
	}

	public String getFnlUpdDt() {
		return fnlUpdDt;
	}

	public void setFnlUpdDt(String fnlUpdDt) {
		this.fnlUpdDt = fnlUpdDt;
	}

	public String getFnlUpderId() {
		return fnlUpderId;
	}

	public void setFnlUpderId(String fnlUpderId) {
		this.fnlUpderId = fnlUpderId;
	}

	public String getPwdValidDt() {
		return pwdValidDt;
	}

	public void setPwdValidDt(String pwdValidDt) {
		this.pwdValidDt = pwdValidDt;
	}

	public String getEncptUserId() {
		return encptUserId;
	}

	public void setEncptUserId(String encptUserId) {
		this.encptUserId = encptUserId;
	}

	public String getPsnlInfoPldgDt() {
		return psnlInfoPldgDt;
	}

	public void setPsnlInfoPldgDt(String psnlInfoPldgDt) {
		this.psnlInfoPldgDt = psnlInfoPldgDt;
	}

	public String getUserRegRsnCode() {
		return userRegRsnCode;
	}

	public void setUserRegRsnCode(String userRegRsnCode) {
		this.userRegRsnCode = userRegRsnCode;
	}

	public String getUserRegRsnDesc() {
		return userRegRsnDesc;
	}

	public void setUserRegRsnDesc(String userRegRsnDesc) {
		this.userRegRsnDesc = userRegRsnDesc;
	}

	public String getSysPicYn() {
		return sysPicYn;
	}

	public void setSysPicYn(String sysPicYn) {
		this.sysPicYn = sysPicYn;
	}

	public String getSysPicRoleDesc() {
		return sysPicRoleDesc;
	}

	public void setSysPicRoleDesc(String sysPicRoleDesc) {
		this.sysPicRoleDesc = sysPicRoleDesc;
	}

	public String getPstnCode() {
		return pstnCode;
	}

	public void setPstnCode(String pstnCode) {
		this.pstnCode = pstnCode;
	}

	public String getAcctNotchkYn() {
		return acctNotchkYn;
	}

	public void setAcctNotchkYn(String acctNotchkYn) {
		this.acctNotchkYn = acctNotchkYn;
	}

	public String getSaveFctCode() {
		return saveFctCode;
	}

	public void setSaveFctCode(String saveFctCode) {
		this.saveFctCode = saveFctCode;
	}

	public String getPhotoUseYn() {
		return photoUseYn;
	}

	public void setPhotoUseYn(String photoUseYn) {
		this.photoUseYn = photoUseYn;
	}
	
	public String getUniqueSeq() {
		return uniqueSeq;
	}
	
	public void setUniqueSeq(String uniqueSeq) {
		this.uniqueSeq = uniqueSeq;
	}
}
